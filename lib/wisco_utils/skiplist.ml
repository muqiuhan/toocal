...
module SNode : sig
  ...
  let create k v l = Node {key = k; value = v; next = Array.init l (fun _ -> Nil)}
...
module Make (Ord : OrderedType) : S with type key = Ord.t = struct
  ...
  let find_linked (sl : 'a t) (key : key) : (int * (key, 'a) node) list * (key, 'a) node =
  ...
  in
    let hdr =
      Printf.sprintf
        "Info: max_level: %d, level: %d, len: %#d\n"
        sl.max_level
        sl.cur_level
        sl.length
  in
